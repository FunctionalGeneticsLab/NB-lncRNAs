...
for (name in colnames(data)[!colnames(data) %in% "FileID"]) {
	pvcor <- cor.test(data[[colname]], data[[name]], na.action = "na.exclude", method="spearman")$p.value
	pvvector <- c(pvvector, pvcor)
	cor <- cor(data[[colname]], use = "complete.obs", data[[name]], method="spearman")
...
write.table(df, file=sprintf("SpearmanCorrelations_%s_%s.table",args[3],args[2]),sep="\t", quote=FALSE)
